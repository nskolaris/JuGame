...
<script>
	...
	function refreshGameData(){
		var text = 'Tiempo por pregunta: '+partida.mp_tiempo_pregunta+'\n';
		text += 'Mi Grupo('+partida.equipo.nombre+'): ';
		$.each(partida.equipo.integrantes,function(id,integrante){
			text += integrante.apellido + ', ';
		});
	...
	function showPregunta(pregunta_id){
		...
		var i = 1;
		$.each(pregunta.opciones,function(id,respuesta){
	...
	function selectRespuesta(respuesta_id){
		...
		if(seconds_timer.running){
			seconds_timer.stop();
			for(var i = 1; i <= 5; i++){
				if(respuesta_id != partida.preguntas[current_pregunta_id].opcion_correcta_id){
				...
				if(EZGUI.components['respuesta_'+i].settings.respuesta_id == partida.preguntas[current_pregunta_id].opcion_correcta_id){
	...
	function addPoints(answer_points,time_points){
		var afterShowScore = function(){
			setTimeout(function(){
				...
				if(respuesta_id_selected != partida.preguntas[current_pregunta_id].opcion_correcta_id){
					title = "Respuesta incorrecta...";
				}
				var comentario = partida.preguntas[current_pregunta_id].comentario;
				if(respuesta_id_selected != 0){
					comentario = partida.preguntas[current_pregunta_id].opciones[respuesta_id_selected].comentario;
				}
				var text = comentario;
